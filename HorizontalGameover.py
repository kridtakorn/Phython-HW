...
mini_img =game.transform.scale(player_img,(25,19))
...
def show_go_screen():
    ...
    draw_text(screen , "Press T  to begin" , 50 , WIDTH / 2 , 360)
...
while running:
    ...
    Death = game.sprite.spritecollide(character,mobs,True,game.sprite.collide_circle)
    if Death:
        for hit in Death:
            character.hp -= 10
            NewMob()
        if character.hp <=10 and character.lives > 0:
            character.hide()
            character.lives -=1
            character.hp =100
    ...
    draw_hp_bar(screen,240,10,character.hp)
    draw_live(screen, 0 , 0 , character.lives , mini_img)
    game.display.flip()
game.quit()   